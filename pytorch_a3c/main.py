...
if __name__ == '__main__':
    ...
    shared_model = ActorCritic(6, config, {'train_resnet': args.train_resnet, 'history_size': args.history_size}, train_resnet=args.train_resnet or config['train_resnet'], use_gpu=use_gpu)
    ...
    if not args.synchronous:
        ...
        for rank in range(0, args.num_processes):
            p = mp.Process(target=train, args=(training_scenes, train_objects, rank, args, shared_model, counter, lock, config,\
                            {'train_resnet': args.train_resnet, 'history_size': args.history_size}, optimizer, use_gpu))
            p.start()
            processes.append(p)
        for p in processes:
            p.join()
        ...
        train(training_scenes, train_objects, rank, args, shared_model, counter, lock, config, \
                {'train_resnet': args.train_resnet, 'history_size': args.history_size}, optimizer, use_gpu)  # run train on main thread
